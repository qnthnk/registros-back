...
def get_customers_list_by_user():
    try:
        ...
        user_id = data.get("id")
        if not user_id:
            return jsonify({"error": "Se requiere el id del usuario en el body JSON."}), 400

        # Consultar los 50 clientes más nuevos filtrados por created_by = user_id
        customers = Customer.query.filter_by(created_by=user_id).order_by(Customer.created_at.desc()).limit(50).all()
        customers_list = [
            {
                "id": customer.id,
                "name": customer.name,
